...
/*   Updated: 2024/04/29 09:29:26 by mzhukova         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int map_validation(char *argv, t_args *args)
{
	...
	if (ft_strncmp(res, ".ber", 4) != 0)
		so_short_error("Incorrect map format!", args);
	check_walls(args);
	find_player(args);
	find_exit(args);
...
int check_walls(t_args *args)
{
	...
	{
		if (args->map[0][i] != '1')
			so_short_error("Incorrect wall", args);
		i++;
	}
	i = 0;
		...
		while(args->map[y][x] != '\n' && args->map[y][x])
		{
			if(args->map[y][x] == 'P')
			{
				if (args->map[y][x] == 'P' && found)
					so_short_error("Two many players!", args);
				args->player_x = x;
				args->player_y = y;
				found = true;
			}
		...
		while(args->map[y][x] != '\n' && args->map[y][x])
		{
			if(args->map[y][x] == 'E')
			{
				if (args->map[y][x] == 'E' && found)
					so_short_error("Two many exits!", args);
				args->exit_x = x;
				args->exit_y = y;
				found = true;
			}
...
int is_rectangular(t_args *args)
{
	...
	while (i < args->line_count)
	{
		if (ft_strlen(args->map[i]) != line_len)
			so_short_error("Map is not a rectangle!", args);
		i++;
	}
	return (1);
}
...
int find_collectibles(t_args *args)
{
	...
	if (!args->collects_found)
		so_short_error("No collectibles found.", args);
	return(1);
}
